...
# FORMAT:
#   resource_name, metric_name, group_name, fixed base value, random value, fraction
#
#   Example for "fixed base value"=100 and 2random value"=10
#   Gives metrics values between 100 and 110
#   If you put "fraction"=f then 
#   Gives metrics values between 0.100 and 0.110
#   If you put "fixed base value"=ITERATIONS then it will use the counter (for PodRestarts for example)
declare -a MY_RES_IDS=(
"qotd-rating (qotd/qotd-rating-76d4964f5-8l8z6):::7QthWgdy4IaSS0KkxFUkYmBCLC0,cpu.user_usage,docker,90,10,f"
"qotd-rating (qotd/qotd-rating-76d4964f5-8l8z6):::7QthWgdy4IaSS0KkxFUkYmBCLC0,memory.used_percentage,docker,90,10,f"
...
echo "   "
echo "   ----------------------------------------------------------------------------------------------------------------------------------------"
echo "     🚀  Inject Synthetic Metrics for Anomaly generation"
echo "   ----------------------------------------------------------------------------------------------------------------------------------------"
...
do
      ...
      do
            ...
            do
                  ...
                  MY_FACTOR=$(echo $value | cut -d',' -f6)

                  if [[ ! $MY_FACTOR == "f" ]]; then
                        export CURRENT_VALUE=$(($RANDOM%$MY_VARIATION+$MY_FIX_VALUE))
                  else
                        export TMP_CURRENT_VALUE=$(($RANDOM%$MY_VARIATION+$MY_FIX_VALUE))
                        export CURRENT_VALUE="0."$TMP_CURRENT_VALUE
                        #echo ":::::"$CURRENT_VALUE
                  fi
