...
oc scale deployment -l release=aiops-topology --replicas=0
oc scale deployment aiopsedge-instana-topology-integrator --replicas=0

oc exec -ti aiops-topology-cassandra-0 -- bash -c "/opt/ibm/cassandra/bin/cqlsh --ssl -u \$CASSANDRA_USER -p \$CASSANDRA_PASS -e \"DROP KEYSPACE janusgraph;\""
...
oc scale deployment -l release=aiops-topology --replicas=1
